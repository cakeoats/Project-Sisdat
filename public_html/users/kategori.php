...
if (isset($_SESSION['user'])) {
	...
	if (mysqli_num_rows($check_user) == 0) {
		...
		if (isset($_POST['delete'])) {
			$post_sid = $_POST['id'];
			$checkdb_service = mysqli_query($db, "SELECT * FROM categories WHERE id = '$post_sid'");
			if (mysqli_num_rows($checkdb_service) == 0) {
				$msg_type = "error";
				$msg_content = "<b>Gagal:</b> Kategori tidak ditemukan.";
			} else {
				$delete_service = mysqli_query($db, "DELETE FROM categories WHERE id = '$post_sid'");
...
?>
 ...
 <?php 
  ...
  <?php echo $msg_content; ?>
										...
										?>
										<div class="col-md-6">
											<a href="<?php echo $cfg_baseurl; ?>user/tipe/add.php" class="pull-right btn btn-info"><i class="mdi mdi-plus"></i> Tambah</a>
										</div> 
										<br><br>
...
if (isset($_GET['search'])) {
	$search = $_GET['search'];
	$query_list = "SELECT * FROM categories WHERE name LIKE '%$search%' AND user_id = '$sess_username' ORDER BY id ASC"; // edit
} else {
	$query_list = "SELECT * FROM categories WHERE user_id = '$sess_username' ORDER BY id ASC"; // edit
}
...
// end paging config
												...
												?>
													<tr>
														...
														<td align="center">
														<a href="<?php echo $cfg_baseurl; ?>user/tipe/delete.php?id=<?php echo $data_show['id']; ?>" class="btn  btn-danger"><i class="mdi mdi-delete"></i></a>
															<a href="<?php echo $cfg_baseurl; ?>user/tipe/edit.php?id=<?php echo $data_show['id']; ?>" class="btn btn-warning"><i class="mdi mdi-table-edit"></i></a>
														</td>
													</tr>
												<?php
												}
