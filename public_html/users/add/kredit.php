	...
	if (mysqli_num_rows($check_user) == 0) {
		...
		$data_wallet = mysqli_fetch_assoc($check_wallet);
			...
			if (empty($post_dompet) || empty($post_category) || empty($post_jumlah)) {
				...
				$insert_kredit = mysqli_query($db, "INSERT INTO transactions (user_id, wallet_id ,wallet, amount,note,type,category,date) VALUES ('$sess_username', '$post_dompet', '$name_wallet', '$post_jumlah','$post_note', 'kredit','$post_category',' $update')");
					}
				if ($insert_kredit == TRUE) {
					$msg_type = "success";
					$msg_content = "<b>Berhasil:</b> Transaksi Kredit berhasil ditambahkan.<br /><b>Jumlah:</b> $post_jumlah<br /><b>Dompet:</b> $name_wallet<br /><b>Tanggal:</b> $update";
				} else {
					$msg_type = "error";
...
?>
						...
						<div class="col-md-12">
							<div class="card">
								<div class="card-header">
									<div class="card-title">Tambah Transaksi Kredit</div>
								</div>
								<div class="card-body">
									<div class="row">
							         <div class="col-md-12">
  ...
  <?php echo $msg_content; ?>
										...
										<select class="form-control" name="category">
														<option value="0">Pilih salah satu...</option>
														<?php
														$check_cat = mysqli_query($db, "SELECT * FROM categories WHERE user_id ='$sess_username' AND type = 'kredit' ORDER BY name ASC");
